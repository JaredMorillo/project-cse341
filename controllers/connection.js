...
const getSingle = async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.status(500).json({ message: 'Error finding your friend.', error: err.message });
    }
};
...
const createConnection = async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({ message: 'Error creating a new friend.', error: err.message });
    }
};
...
const updateConnection = async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.status(500).json({ message: 'Error updating your friend.', error: err.message });
    }
};
...
const deleteConnection = async (req, res) => {
    ...
    try {
        ...
        if (result.deletedCount > 0) {
            res.status(200).json({ message: 'Your friend was deleted successfully' });
        } else {
            res.status(404).json({ message: 'Friend not found' });
        }
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Error deleting your friend.', error: err.message });
    }
};
